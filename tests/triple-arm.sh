...
qemu-arm ./scc1 -o $S2-bin $SRC
qemu-arm ./scc  -o $S3-bin $SRC
...
qemu-arm ./scc1 -S $SRC && cat $ASM >$S2-dump && tar cf $S2-asm.tar $ASM && rm $ASM
...
qemu-arm ./scc1 -c $SRC && tar cf $S2-obj.tar $OBJ && ./scc1 -o $S2-bin $OBJ
